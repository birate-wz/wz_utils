...
typedef enum { PROCESS, RESOURCE}Type;
...
vertex *create_vertex(source_type type) {
...
int search_vertex(source_type type) {
	int i = 0;
	for (i = 0; i<tg->num; i++) {
...
void add_vertex(source_type type) {
...
int add_edge(source_type from, source_type to) {
	add_vertex(from);
	add_vertex(to);
...
int verify_edge(source_type i, source_type j) {
...
int remove_edge(source_type from, source_type to) {
